...
export default function FlightPanel({ scraped, phase1Done, onApprove, selectedFlight, setSelectedFlight, status }) {
  ...
  const allFlights = scraped?.all_flights || []
  const trains  = scraped?.trains || []
  const allTrains = scraped?.all_trains || []
    ...
    <div style={s.panel}>
      ...
      {(status === 'running' && !phase1Done) ? (
        ...
        <>
          {flights.length > 0 && (
            <div style={s.section}>
              <div style={s.sectionTitle}>⭐ AI Selected Flights</div>
              <div style={s.grid}>
          ...
          {allFlights.length > 0 && (
            <div style={s.section}>
              <div style={s.sectionTitle}>🛩️ All Available Flights (Top 10)</div>
              <div style={s.grid}>
          ...
          {trains.length > 0 && (
            <div style={s.section}>
              <div style={s.sectionTitle}>⭐ AI Selected Trains</div>
              <div style={s.grid}>
          ...
          {allTrains.length > 0 && (
            <div style={s.section}>
              <div style={s.sectionTitle}>🚂 All Available Trains (Top 10)</div>
              <div style={s.grid}>
                {allTrains.map(renderTrainCard)}
              </div>
            </div>
          )}
